...
var myDB = {
                ...
                if (/cache=true/.test(key)) {
                    ...
                    myDB.getStore(db, table).get(key).onsuccess = function (e) {
                        if (e.target.result) {
                            ...
                            debug && (_console6 = console).log.apply(_console6, logPrefix.concat(['Using NetworkFirst to is responding to:', key]));
                            obj.data = e.target.result.data || {};
                            resolve(obj);
                        } else {
                            var _console7;

                            debug && (_console7 = console).log.apply(_console7, logPrefix.concat(['no data', 'Using NetworkFirst to is responding to:', key]));
                            resolve();
                        }
                    };
                } else {
    ...
    close: function close(_dbName) {
        ...
        delete myDB[_dbName];
        debug && (_console8 = console).log.apply(_console8, logPrefix.concat([_dbName + ' Connection has already been closed']));
    },
    deleteDatabase: function deleteDatabase(_dbName) {
        var _console9;

        window.indexedDB.deleteDatabase(_dbName);
        debug && (_console9 = console).log.apply(_console9, logPrefix.concat([_dbName + ' has already delete']));
    },
        ...
        return new Promise(function (resolve) {
            try {
                ...
                result.onsuccess = function () {
                    ...
                    resolve(true);
                    debug && (_console10 = console).log.apply(_console10, logPrefix.concat(['[' + db.name + '] table name:[' + table + '] has already clear']));
                };
            } catch (error) {
                console.error(error);
                resolve(false);
...
var _IDB = {
    init: function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime.mark(function _callee3(dbName, table, version, using) {
            var db;
